...
class DbTable extends Table {
    ...
    var $table;       // SQL command
    var $where;        // ID
    var $fields;    //fields of sql query
    ...
    function DbTable($db, &$formsgeneration, $table, $where, $fields = '*')
    {
...
//         debug_backtr('DBTABLE');
    ...
    * @param  exclude: do not include this columns in fieldList
    * @param  noAlias: if set to true "AS table%column" is not generated
    ...
    */
    ...
    function getFieldListArray($tablename = '',$exclude = array(), $include = array(), $noAlias = false)
    {
        ...
        if ( !empty($tablename))
        {
            ...
            {
//              print("TABLENAME: $tablename, colArr[$i]: $colArr[$i], Alias: $noAlias<BR>");
    ...
    function showRecordDetails($edit = false, $title='')
    {
        ...
        for ( $i=0; $i < $rs->FieldCount(); $i++ )
        {
            ...
            if ( strtolower($name['raw']) == 'id' ||
                 strtolower($name['raw']) == 'memberid')
            {
...
/*                $errTxt = $this->formsgeneration->AddInput(array(
            ...
            else
            {
                ...
                {
                    debug("DBTABLE", "[DBTABLE: showRecordDetails] NAME: {$name['raw']} VALUE: $value, TYPE: $name[type]");
                    switch ( $name['type'] )
                    {
                        case 'myref' :
...
//             $this->formsgeneration->LoadInputValues(true);
    ...
    function checkValue($fieldName, $fieldValue, $checkArr)
    {
        ...
        if ( isset($checkArr[$fieldName['raw']]) )
        {
            ...
            {
                $APerr->setError(lang('Invalid format for field ') . $fieldName['pretty'] . ": '$fieldValue' <=> " . $checkArr[$fieldName['raw']]);
                return false;
            }
        }
        else
    ...
    function newRecord($presetVals=array(), $presetEditVals=array(), $title = '')
    {
        ...
        for ( $i=0; $i < $rs->FieldCount(); $i++ )
        {
            ...
            $editVal = empty($presetVals[$name['raw']]) ?
                            ( empty($presetEditVals[$name['raw']]) ?
                                    '' :
                                    $presetEditVals[$name['raw']])
                            : $presetVals[$name['raw']];
        ...
        */
        ...
        if ( $rs->RecordCount() == 1 )
        {
            ...
            for ($i=0; $i < $rs->FieldCount(); $i++ )
            {
                ...
                if (strtolower($field_name['raw']) == 'id'||
                    strtolower($field_name['raw']) == 'memberid' )
                {
                    continue;
                }
...
//                         case 'blob':
                        ...
                        case 'password':
                            ...
                            if ($pw != '' && $pw != '*****' )
                            {
                                ...
                                // Delete password by entering a single space !!!!
                                if ( $pw == ' ' )
                                {
                                    $pw = '';
                                }
                        ...
                        case 'mylink':
                            debug_r("DBTABLE",  $_GET, "[DBTABLE, updateRecord, mylink]: $field_name[raw] = $value, 'GET=");
                            if ( $this->formsgeneration->GetCheckedState($field_name['raw'] . '_DELETE') )
                            {
                                ...
                                $this->formsgeneration->SetInputProperty($field_name['raw'] . "_DELETE",'ApplicationData','');
                            ...
                            elseif ( $value != '' )
                            {
                                ...
                                if ( $destFileName != '' )
                                {
                                    ...
                                    $this->formsgeneration->SetInputProperty($field_name['raw'] . "_DELETE",'ApplicationData',$destFileName);
                                }
                            }
                            break;
            ...
            if ( $error === false )
            {
                ...
                {
                    $APerr->setFatal(__FILE__,__LINE__,$this->db->errormsg(),"SQL: $sql",lang('Please correct errors'));
                }
                else
                {
                    logEntry('UPDATE', $sql);
